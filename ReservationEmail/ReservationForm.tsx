...
export default function ReservationForm() {
  ...
  const sendTestEmail = () => {
    const serviceID = 'service_dhbqzr8';
    const templateID = 'template_mc8cc6i';
    const userID = '_JxTD0IIJZWKI6-QX'; // Public API key

    const testEmailParams = {
      customerName: 'Test User',
      email: 'test@example.com',
      items: {
        emailContent: '<h2>This is a test email</h2><p>Testing EmailJS integration.</p>',
        subject: 'Test Email'
      }
    };

    emailjs.send(serviceID, templateID, testEmailParams, userID)
      .then((response) => {
        console.log('Test email sent successfully:', response.status, response.text);
        toast({
          title: language === "en" ? "Success!" : "¡Éxito!",
          description: language === "en" ? "Test email sent successfully" : "Correo de prueba enviado correctamente"
        });
      })
      .catch((error) => {
        ...
        toast({
          ...
          description: error.message || (language === "en" ? 
            "There was a problem sending the test email. Please try again." : 
            "Hubo un problema al enviar el correo de prueba. Por favor intenta nuevamente."),
          variant: "destructive"
        });
      });
  };
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Ahora, enviar correo usando EmailJS
      sendTestEmail(); // Call the test email function
